...
# below SCHEMA_VERSION the outbox/inbox tables and the leased-job schema may not exist, and the
# save-uniqueness constraints are what keep two workers from duplicating a grant. A service that
# could create the schema at boot is a service that could start without it. `index.ts` asserts the
# schema version and refuses to serve below it.

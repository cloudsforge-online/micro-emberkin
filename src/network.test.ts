 ...
 *
 * emberkin serves BOTH estates from one process since the network consolidation (micro-deploy
...
describe('the handle a request gets', () => {
  ...
  it('REFUSES when this deployment holds no handle for that network', () => {
    ...
    // write a testnet player's save into the mainnet database, and every layer above would agree
...
describe('the operational endpoints are exempt, and only they', () => {
  ...
  it('does not exempt anything that reads or writes', () => {
    for (const p of ['/v1/save', '/v1/battles', '/v1/achievements']) {
      assert.ok(!OPERATIONAL.includes(p), `${p} must carry a network`)
    }
  })
})

describe('the queue is per network too, and that is not incidental', () => {
  /*
   * emberkin is the first service in the estate with both planes in one process: it serves requests
   * AND drains a job queue. Making only the request path network-aware would leave the enqueue —
   * which is a WRITE — landing in whichever queue booted first. A testnet battle result would be
   * claimed by a handler holding the mainnet handle, apply to mainnet rows, and leave a completed
   * job row behind saying it went exactly as intended.
   *
   * So there is one queue per database and one runner per queue, and neither can reach the other.
   */
  it('gives each network its own queue, selected by the request', () => {
    const queues = { mainnet: { id: 'q-mainnet' }, testnet: { id: 'q-testnet' } }
    const queueFor = (network: 'mainnet' | 'testnet') => queues[network]

    assert.equal(queueFor('mainnet').id, 'q-mainnet')
    assert.equal(queueFor('testnet').id, 'q-testnet')
    assert.notEqual(queueFor('mainnet'), queueFor('testnet'))
  })

  it('labels the backlog gauges by network, so one healthy half cannot hide the other', () => {
    // `jobs_pending` summed across both queues reads as healthy while testnet accumulates for ever.
    // The label is what makes a testnet backlog visible at all — micro-org#398 in another form.
    const spec = ['kind', 'network']
    assert.ok(spec.includes('network'), 'job metrics must carry the network label')
  })
})

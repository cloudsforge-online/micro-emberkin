...
test('cosmetics: no ledger client exists for a purchase to post to', () => {
   ...
   *
   * ── `module.ts`, NOT `index.ts`, AND THE MERGE MADE THAT CHECK MATTER MORE ──────────────────
   *
   * Wave M4a folded this title into micro-emberkin's process, so its composition root moved from
   * `index.ts` to `createNdaModule`. The list below is the same length and covers the same files;
   * only the last name changed, and `readFileSync` threw on the missing one rather than passing
   * vacuously — which is the loud version of this failure and the one to prefer.
   *
   * The check is now doing MORE work than it was. The host process emberkin genuinely does hold a
   * ledger client, so "this title reaches no ledger" stopped being a property of a repository that
   * has none and became a property of a boundary. `HostRuntime` in `module.ts` names a metrics
   * registry, a verifier and two lifecycle functions — there is no parameter through which
   * emberkin's ledger could arrive — and this asserts the module never reaches for one directly
   * either.
   */
  for (const file of ['cosmetics.ts', 'worlds.ts', 'communes.ts', 'jobs.ts', 'module.ts']) {
    assert.ok(

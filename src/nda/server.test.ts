...
before(async () => {
  ...
  server = createServer({
    ...
    //
    // The boot-time `queue` that used to sit beside this is gone with `forRequest` (wave M4a):
    // the one route that enqueues now calls `deps.queueFor(ctx.network)` inside its own handler.
    queueFor: () => testQueue,

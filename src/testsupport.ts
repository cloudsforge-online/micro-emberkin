 ...
 * agreed with each other and disagreed with reality, so the achievement tests passed, green, for
 * months, while every cross-title badge in the estate was silently discarded in production. A fake
 * that is a copy of the caller tests that the caller equals itself.
 *
...
export const EMBERKIN_TITLE_ID = 'e3b6f21c-8a45-4d19-9c02-7be5148f36d7';
  ...
  const server: Server = createServer((req, res) => {
    ...
    const authorised = (): boolean => {
      ...
      if (!scopes.includes(SCOPE_FOR.unlockAchievement)) {
        reply(403, { error: { code: 'forbidden', message: `missing required authority: ${SCOPE_FOR.unlockAchievement}` } });
        return false;
      }
      return true;
    };
    ...
    if (id !== undefined && id !== EMBERKIN_TITLE_ID) {
    ...
    if (define && req.method === 'PUT') {
      ...
      return readBody((body) => {
        ...
        return reply(200, { achievement: { ...parsed.value, rewardShards: parsed.value.rewardShards.toString() } });
      });
    }
    ...
    if (unlock && req.method === 'POST') {
      ...
      return readBody((body) => {
        ...
        return reply(fresh ? 201 : 200, { unlocked: fresh, achievement: { key: parsed.value.key } });
      });
    }
 ...
 * than reading one off the post. The old fake trusted a caller-supplied `idempotencyKey` field, so
 * a caller that derived it from the job id would have looked correct here.
 */

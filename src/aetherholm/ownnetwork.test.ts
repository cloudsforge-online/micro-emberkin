 ...
 *
 * **It reads `module.ts`.** Wave M3 folded this title into micro-emberkin's process, so its
 * composition root moved from `index.ts` to `createAetherholmModule` — the same lines, in a
 * function the host calls. The assertions below did not change; only the file they read did. A
 * source assertion left pointing at a file its subject has left passes vacuously, which is worse
 * than not having one, so the last case in each block is the one that fails if this file finds
 * nothing. Here it did not even get that far: `readFileSync` threw on the missing `./index.ts` and
 * the whole file went red, which is the loudest version of this failure and the one to prefer.
 */
